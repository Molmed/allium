...
def groupings(DNAm = False):
    if DNAm == True:
        subtype_groups = {'aneuploidy': ['HeH', 'low HeH', 'iAMP21', 'Hypo'], 'ph-group': ['t(9;22)', 'ph-like'],
        ...
        subtype_groups = {'aneuploidy': ['HeH', 'low HeH', 'iAMP21'], 'ph-group': ['t(9;22)', 'ph-like'],
                     'ETV6-group': ['t(12;21)', 't(12;21)-like'],  'T-ALL': ['T-ALL'], 'DUX4-r': ['DUX4-r'],
           'PAX5alt': ['PAX5alt'], '11q23/MLL': ['11q23/MLL'], 'ZNF384-r': ['ZNF384-r'], 't(1;19)': ['t(1;19)'], 
            'MEF2D-r':['MEF2D-r'], 'PAX5 p.Pro80Arg': ['PAX5 p.Pro80Arg'], 'NUTM1-r': ['NUTM1-r'], 'Control': ['Control']
           }
...
def trainingNSC(designdf, designpheno, allsubs, unique_genedf, subtypecol, ids, signature_mode = 'all',
                DNAm_data = False):
    """ 
    Arguments:
    --designdf: pandas dataframe num_samples x num_features for the dataset used to design the classifier
    --designpheno: pandas dataframe num_samples x num_features for the designdf
    --allsubs: subtype column name
    ...
    --ids: the ID name column for the signatures from the unique_genedf
    ...
    -- DNAm_data: If True then imputation will take place for the DNAm cohort, else no imputation will be performed

    Returns:
    
    --model: A dictionary containing a fitted NSC classifier model per subtype
    --imp: if DNAm_data equals True, then an imputer will be returned to impute the missing values

    """
    model = {}
    ## Missing data imputation for DNA methylation##
    if DNAm_data == True:
        designdf = designdf[unique_genedf[ids].to_list()]
        imp = SimpleImputer(missing_values=np.nan, strategy='median')
        imp.fit(designdf)
        designdf = pd.DataFrame(imp.transform(designdf), columns = designdf.columns, index = designdf.index)
        subtype_groups = groupings(DNAm = True)
    else: 
        subtype_groups = groupings(DNAm = False)
    
    for subtype in list(subtype_groups.keys()): # The subtype loop  
        print('Starting with subtype group {}'.format(subtype))
        ############ ONE vs REST training ####################
        # Create a subtype column one-vs rest per subtype
        subtypes = []
        for s in designpheno[allsubs]:
            if s in subtype_groups[subtype]:
                subtypes.append(subtype)
            else:
                subtypes.append('other')              
        designpheno['subtypes'] = subtypes           
        encoder = {'other': 0, subtype: 1}
        # encode the data
        designpheno.loc[:,'classes'] = designpheno.subtypes.map(encoder)
        ...
        clfall = NearestCentroid()
        clfall.fit(designdf[sub_genes], designpheno.classes)
        model[subtype] = clfall
        ...
        if len(separation_subs) == 2:
            ...
            newdf = designpheno[designpheno[allsubs].isin(separation_subs)].copy()
            newdf['subtypes'] = newdf[allsubs]
            Xnew = designdf.reindex(newdf.index)
            #label_encoder = {separation_subs[0]: 0, separation_subs[1]: 1}
            label_encoder = {}
            for i, sub in enumerate(separation_subs):
                label_encoder[sub] = i
            newdf.loc[:,'classes'] = newdf.subtypes.map(label_encoder)
            if signature_mode == 'all':
                sub_genes = unique_genedf[ids].to_list() # subtype specific signatures
            else:
                sub_genes = unique_genedf[unique_genedf[subtypecol] == subtype_sep][ids].to_list() # subtype specific signatures
            clfall = NearestCentroid()
            clfall.fit(Xnew[sub_genes], newdf.classes)
            model[subtype_sep] = clfall
        ############################### Multiclass classifier for groups with >2 subtype members ###################
        elif len(separation_subs) > 2: # for groups with more than 2 members
...
def predictionsNSC(model, discoverydf, discoverypheno, clinicaldatalist, unique_genedf, 
